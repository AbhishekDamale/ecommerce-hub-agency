...
const destinations = [
  ...
  { number: "04", label: "Team", title: "Meet the people behind it.", copy: "See the founders and the design lead building every next move together.", href: "/team", icon: Users, tone: "coral" },
];

const studioLinks = [
  { label: "About", copy: "Read the studio story and how we work.", href: "/studio#about", icon: Info },
  { label: "Team", copy: "Meet Abhishek and Yash, the founders behind the studio.", href: "/team", icon: Users },
...
export default function ExplorePage() {
  return <div className="explore-page"><header className="explore-nav"><Link className="detail-back" href="/">e_commerce<span>.hub</span></Link><Link className="detail-home-link" href="/"><ArrowDownRight size={16} /> Back to home</Link></header><main><section className="explore-hero"><div><p className="eyebrow"><span>e_commerce.hub / 02</span> Explore the studio</p><h1>Choose the<br /><em>next page.</em></h1><p>Two home screens, no carousel. Use the heading buttons below to open a dedicated page with the full detail.</p></div><div className="explore-index"><span>Navigation / 01—04</span><strong>02</strong><small>static studio directory</small></div></section><section className="explore-destinations" aria-label="Explore e_commerce.hub"><div className="explore-section-heading"><p className="eyebrow"><span>Directory</span> The important bits</p><h2>Open the section<br /><em>you came for.</em></h2></div><div className="explore-grid">{destinations.map((destination) => { const Icon = destination.icon; return <Link className={`explore-card explore-card-${destination.tone}`} href={destination.href} key={destination.number}><div className="explore-card-top"><span>{destination.number} / {destination.label}</span><Icon size={20} /></div><div><h3>{destination.title}</h3><p>{destination.copy}</p></div><span className="explore-card-link">Open page <ArrowUpRight size={16} /></span></Link>; })}</div></section><section className="explore-studio-links" aria-labelledby="studio-links-title"><div className="explore-section-heading"><p className="eyebrow"><span>Directory / 05—09</span> More from the studio</p><h2 id="studio-links-title">Keep the rest<br /><em>within reach.</em></h2></div><div className="studio-links-grid">{studioLinks.map((link) => { const Icon = link.icon; return <Link className="studio-link-card" href={link.href} key={link.label}><div><Icon size={18} /><span>{link.label}</span></div><p>{link.copy}</p><ArrowUpRight size={17} /></Link>; })}</div></section><section className="explore-footer-cta"><span>Need a recommendation?</span><h2>Start with the<br /><em>useful question.</em></h2><a href="/studio#contact" className="button button-coral">Talk to the studio <ArrowUpRight size={16} /></a></section></main><footer className="detail-page-footer"><div><strong>e_commerce<span>.hub</span></strong><p>Marketing, social growth, and websites for the next move.</p></div><Link href="/">Return to homepage <ArrowDownRight size={16} /></Link></footer></div>;
}
